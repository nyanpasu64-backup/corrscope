...
class CorrelationTrigger(MainTrigger):
    ...
    The data read from the input _wave (length L) is longer than the correlation
    buffer (length A+B), to ensure that when sliding the buffer across the wave,
    no edge effects are encountered. This is inspired by overlap-save convolution,
    but adapted to cross-correlation.
    """

    cfg: CorrelationTriggerConfig

    A: int
    """Number of subsamples on buffer/step's left (negative) side."""
    B: int
    """Number of subsamples on buffer/step's right (positive) side. Equals A.
    buffer_nsamp = A+B
    """

    L: int
    """Number of subsamples fetched from input _wave, longer than the buffer/step. 
    Approximately equal to 1.5 * (A+B). """
    ...
    _prev_slope_finder: Optional[np.ndarray]
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.L = int((self.A + self.B) * (1 + self.cfg.trigger_diameter))
    ...
    def get_trigger(self, index: int, cache: "PerFrameCache") -> TriggerResult:
        ...
        # Convert A and L to full samples (not trigger subsamples) when indexing into
        # _wave.
        stride_A = stride * self.A
        stride_L = stride * self.L

        smp_per_frame = self._smp_per_frame
        begin = max(index - stride_A - smp_per_frame, index - stride_L // 2)

        # Get data (1D, downmixed to mono)
        data = self._wave.get_padded(begin, begin + stride_L, stride)
        assert data.size == self.L, (data.size, self.L)
